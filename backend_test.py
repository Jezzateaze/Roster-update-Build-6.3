...
class ShiftRosterAPITester:
    def __init__(self, base_url="https://staffshift-app.preview.emergentagent.com"):
...
def main():
    ...
    print("\n" + "=" * 60)
    print("🔄 Running ALLOW OVERLAP FUNCTIONALITY TESTS...")
    print("🎯 Testing: Manual Allow Overlap Override for 2:1 Shifts")
    print("=" * 60)
    
    allow_overlap_tests_passed = 0
    for test in allow_overlap_tests:
        try:
            result = test()
            if result:
                allow_overlap_tests_passed += 1
        except Exception as e:
            print(f"❌ Allow overlap test failed with exception: {str(e)}")
    ...
    print(f"🔄 Allow Overlap Tests: {allow_overlap_tests_passed}/{len(allow_overlap_tests)} allow overlap tests passed")
    ...
    allow_overlap_success = allow_overlap_tests_passed == len(allow_overlap_tests)
    ...
    if allow_overlap_success:
        print("🎉 All allow overlap functionality tests passed!")
    else:
        print("⚠️  Some allow overlap functionality tests failed.")
